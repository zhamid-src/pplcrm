...
2. **Design-review gate (design principles §9):** two surfaces here are novel page types —
   ...
   primary, uppercase kickers, pins-and-annotations format) and present it to the owner for review.
   Build the backend (Phases 1–3) while waiting; do not code those two UIs before the mock
   exists.
3. **If the build is currently broken by unrelated work in progress, do not stop.** The owner has
...
1. **Phase 0 — design gate:** mockup artifact for the plan page + volunteer page → the owner.
