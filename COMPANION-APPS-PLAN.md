...
pplCRM has two volunteer-facing companion web apps, opened from a personal link, no account:
...
- `verify/start`: person must have the requested channel on file (email / normalizable mobile); code via `enqueueMail` (use the `.otp-code` template style) or `enqueueSms` ("pplCRM code: 123456 — expires in 10 minutes"). Because codes must survive rollbacks correctly, generate + store the hash and enqueue in one transaction.
- `verify/confirm`: constant-time-ish hash compare, attempts++, on success clear code fields, `verified_at=now()`, status `invited→verified`, mint session (`generateToken`/`hashToken`, 30d), and **if this created a newly-verified volunteer, enqueue an email to all tenant admins/owners** ("Jordan Rivera verified their phone and is waiting for approval → pplCRM → Volunteer access") in the same transaction.
