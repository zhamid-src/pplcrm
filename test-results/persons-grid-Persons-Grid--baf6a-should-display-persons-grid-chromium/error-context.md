...
- Name: persons-grid.spec.ts >> Persons Grid >> Grid Display >> should display persons grid
- Location: apps/frontend-e2e/src/persons-grid.spec.ts:121:5
...
Locator: locator('pc-persons-grid pc-datagrid')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('pc-persons-grid pc-datagrid')
...
```ts
  ...
  26  |     });
  27  | 
  28  |     // 2. Mock persons.getAllWithAddress
  29  |     await page.route(/\/persons\.getAllWithAddress/, async (route) => {
