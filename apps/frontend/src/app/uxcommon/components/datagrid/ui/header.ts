...
export class DataGridHeaderComponent {
  ...
  public getFilterOptionsForCol = input<(col: ColDef) => string[] | null>((c) => this.grid?.getFilterOptionsForCol(c) ?? null);
  ...
  public isOptionChecked = input<(field: string, option: string) => boolean>((f, o) => !!this.grid?.isOptionChecked(f, o));
  ...
  public onHeaderFilterInput = input<(field: string, value: any) => void>((f, v) => this.grid?.onHeaderFilterInput(f, v));
  ...
  public onSelectionResizeTouchStart = input<(ev: TouchEvent) => void>((e) => this.grid?.onSelectionResizeTouchStart(e));
  ...
  public sortIndicatorForHeader = input<(h: any) => PcIconNameType>((h) => (this.grid?.sortIndicatorForHeader(h) as any) ?? ('none' as PcIconNameType));
