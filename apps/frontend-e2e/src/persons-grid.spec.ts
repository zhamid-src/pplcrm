...
test.describe('Persons Grid', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto('/console/persons');
  ...
  test.describe('Grid Display', () => {
    ...
    test('should display column headers', async ({ page }) => {
      ...
      await expect(page.locator('.ag-header-cell-text:has-text("Name"), .ag-header-cell:has-text("First Name")')).toBeVisible();
      await expect(page.locator('.ag-header-cell-text:has-text("Email"), .ag-header-cell:has-text("email")')).toBeVisible();
      await expect(page.locator('.ag-header-cell-text:has-text("Mobile"), .ag-header-cell:has-text("mobile")')).toBeVisible();
    });
  ...
  test.describe('Address Confirmation', () => {
    test('should show confirmation dialog for address changes', async ({ page }) => {
      ...
      const addressCell = page.locator('.ag-row .ag-cell:has-text("street"), .ag-row .ag-cell[col-id*="address"]').first();
    ...
    test('should allow confirming address changes', async ({ page }) => {
      ...
      if (await addressCell.count() > 0) {
        ...
        if (await confirmDialog.count() > 0) {
          ...
          const confirmButton = confirmDialog.locator('button:has-text("Confirm"), button:has-text("Yes"), .btn-primary');
