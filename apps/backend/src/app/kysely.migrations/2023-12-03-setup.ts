...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema.createIndex('authusers_email_index').on('authusers').column('email').execute();
  ...
  await db.schema
    ...
    .addForeignKeyConstraint('fk_createdby_id', ['createdby_id'], 'authusers', ['id'])
    .execute();
  ...
  await db.schema
    ...
    .addColumn('refresh_token', 'uuid', (col) => col.defaultTo(sql`gen_random_uuid()`).unique())
    .addColumn('user_id', 'bigserial', (col) => col.notNull())
    .addColumn('tenant_id', 'bigint', (col) => col.notNull())
    .addColumn('created_at', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn('last_accessed', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
  ...
  await db.schema
    ...
    .addColumn('created_at', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn('updated_at', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
    .addForeignKeyConstraint('fk_campaigns_tenant_id', ['tenant_id'], 'tenants', ['id'])
    .addForeignKeyConstraint('fk_admin_id', ['admin_id'], 'authusers', ['id'])
    .addForeignKeyConstraint('fk_createdby_id', ['createdby_id'], 'authusers', ['id'])
  ...
  await db.schema
    ...
    .addColumn('created_at', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn('updated_at', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
    .addForeignKeyConstraint('fk_map_campaigns_tenant_id', ['tenant_id'], 'tenants', ['id'])
    .addForeignKeyConstraint('fk_user_id', ['user_id'], 'authusers', ['id'])
  ...
  await db.schema
    ...
    .addColumn('created_at', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn('updated_at', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
    .addForeignKeyConstraint('fk_househods_tenant_id', ['tenant_id'], 'tenants', ['id'])
    .addForeignKeyConstraint('fk_campaign_id', ['campaign_id'], 'campaigns', ['id'])
    .addForeignKeyConstraint('fk_createdby_id', ['createdby_id'], 'authusers', ['id'])
  ...
  await db.schema
    ...
    .addColumn('created_at', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn('updated_at', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
    .addForeignKeyConstraint('fk_persons_tenant_id', ['tenant_id'], 'tenants', ['id'])
    .addForeignKeyConstraint('fk_campaign_id', ['campaign_id'], 'campaigns', ['id'])
    .addForeignKeyConstraint('fk_createdby_id', ['createdby_id'], 'authusers', ['id'])
  ...
  await db.updateTable('profiles').set({ tenant_id: 1, auth_id: 1 }).where('id', '=', 1).execute();

  // Update the auth user with the tenant id
  await db.updateTable('authusers').set({ tenant_id: 1 }).where('id', '=', 1).execute();
  ...
  await db.schema
    .alterTable('profiles')
    .addForeignKeyConstraint('fk_profiles_tenant_id', ['tenant_id'], 'tenants', ['id'])
    .execute();
