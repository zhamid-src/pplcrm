...
description: "Conventions for building component-internal state, forms, loading, and icons in the Angular 22 frontend (signals-only, the form() signal-forms helper, createLoadingGate, pc-icon). USE WHEN adding or editing a component under apps/frontend/src/app/**, wiring an add/edit form, gating a spinner on a tRPC call, or placing a pc-icon. EXAMPLES: 'add an edit form for teams', 'why is my form always invalid', 'the spinner flickers on fast responses', 'wrap this save in a loading gate', 'what values can pc-icon name take', 'use input.required for this component input'."
---
...
## The one thing CLAUDE.md gets wrong: `form()` is Angular, not internal

CLAUDE.md calls `form()` "an internal form() helper — not Angular's ReactiveFormsModule." It is
**not internal** — there is no `export function form(` anywhere in the repo. It is Angular's
signal-forms API, imported from `@angular/forms/signals`:
...
Other validators used in the repo (grep-confirmed) live in the same module: `submit`,
`minLength`, `pattern`, `validate`, `validateStandardSchema`, and the `FormField` type. Import
them from `@angular/forms/signals` — do not hand-roll them.

## Form pattern: signal payload → `form()` → `[formField]` → `.invalid()`

Three moving parts. From `user-edit.ts:42-56`:
...
call parens), not the payload value. From `user-edit.html:47-63`:
...
field's error state — `input.ts:26` shows they read `formField()().invalid()` and
`.errors()` internally, so you do not write per-field error markup yourself.

Validate on submit by **calling** the form to get its root state. From `user-edit.ts:90-104`:
...
resolve" mistake. `.invalid()` is used across ~29 call sites in the frontend.

## Loading gate: suppress spinner flicker on sub-300ms responses

`createLoadingGate()` (`libs/uxcommon/src/loading-gate.ts:10`) returns `{ begin, visible }`.
...
Wire-up and call site from `user-edit.ts:33-34, 180-203`:
...
`end()` in `finally` is mandatory. It is idempotent (guarded by an internal `done` flag,
`loading-gate.ts:61-62`), so calling it twice is safe; never calling it leaves `pendingCount`
above zero and the gate never hides. Note: `loading` gates the initial skeleton; use a separate
`saving`/`resettingPassword` boolean signal (`user-edit.ts:35,132`) to disable buttons during
...
- `name` is `input.required<PcIconNameType>()` (`icon.ts:29`) — required, and must be a valid
  `PcIconNameType`.
- `size` is `input<number>(6)` (`icon.ts:31`), default 6.

`PcIconNameType = keyof typeof icons` (`icons.index.ts:7`). The `icons` object has ~158 entries
— do **not** memorize or paste a subset here, it will rot. Read the live list in
`libs/uxcommon/src/components/icons/icons.index.ts` (mostly Heroicons; `unknown` is the
fallback). A bad name silently falls back to the `unknown` icon (`icons.index.ts:18-27`).

Whole integers only for `[size]`. The size renders as Tailwind classes `w-${size} h-${size}`
(`icon.ts:82`), so `[size]="4"` → `w-4 h-4`. Do not pass decimals (`[size]="3.5"`) and do not
put width/height/`size-*` utilities on `<pc-icon>` — its class-scrubber strips them
(`icon.ts:64`). Real usage: `<pc-icon name="lock-closed"></pc-icon>` (`user-edit.html:33`).
...
never via the constructor. From `user-edit.ts:25-48`:
...
body should not re-subscribe to signals it reads (`user-edit.ts:72-83`).
