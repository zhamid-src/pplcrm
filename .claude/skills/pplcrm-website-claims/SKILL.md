...
| Donation processors + 1% Stripe platform fee (none via own Helcim account)                                                    | donations module; `DONATIONS_PLATFORM_FEE_PERCENT` in env                           | `eula-content.ts` §9, `privacy-content.ts` (subprocessors), `security-content.ts` (Payments), `pricing-page.html` (notes), `faq-page.ts` (Pricing)                |
...
| Subprocessor list (Azure, Cloudflare, Stripe, Helcim, Postmark, SendGrid, Twilio, Google Maps, Google/Microsoft sync, Zapier) | integration modules + `.env.production.example`                                     | `privacy-content.ts` (Service providers). Adding/removing a vendor = edit this list, same commit                                                                  |
